...
sign_app.py
-----------
Real-time Hand Sign Language Detector - desktop GUI (CustomTkinter).

Pipeline per frame:
  webcam frame -> MediaPipe Hands (landmark detection) -> crop hand region,
  draw the same landmark-skeleton style used in the training dataset ->
  feed crop into the trained CNN (sign_model.keras) -> predicted class +
  confidence -> smoothing / debouncing -> optional auto-append to a
  sentence builder.

Run:
    python sign_app.py
Requires sign_model.keras and labels.json (produced by train_model.py)
